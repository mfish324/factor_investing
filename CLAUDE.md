...
- **Trading layer** (`trading/`) places real Alpaca orders (paper or live, gated by `config.ALPACA_PAPER` + `ALPACA_LIVE_TRADING_CONFIRM=YES` for live). `PortfolioManager` caps any position at `MAX_POSITION_PCT`, holds back `CASH_BUFFER_PCT`, and aborts (`TurnoverGuardError`) a rebalance that would trade more than `MAX_REBALANCE_TURNOVER_PCT` of the portfolio unless `force=True` — a guard against a data bug silently liquidating/rebuilding the book, not portfolio theory. Orders are verified filled via `wait_for_order_fill`; unfilled ones land in `RebalanceResult.trades_unfilled` rather than counting as executed. `trading/alerts.py` emails on rebalance failure/aborted/failed-or-unfilled trades (no-ops if SMTP env vars aren't set). `trading/scheduler.py` has no in-process loop — `--check-and-run` is meant to be invoked once daily by an external scheduler (Windows Task Scheduler on this machine, task `FactorInvesting Rebalance`) and decides for itself whether today is a rebalance day, reading last-rebalance state back from the strategy's own JSONL trade log (`results/trading_logs/<model>_trades.jsonl`) so due-date tracking survives separate process invocations. `main.py trade reconcile` diffs actual Alpaca holdings against `ShadowDB`'s expected holdings (shadow tracking is a parallel backtest-engine simulation, not a reconciliation against real fills, so the two can drift). See [[trading-layer-hardening]] for the full buildout and incident notes below.
...
### Trading layer hardening (2026-08-16/17)

Before this, `trading/` could already place real Alpaca orders but had no risk guardrails, no fill verification, a fragile paper/live toggle (`paper=True` hardcoded at 6+ call sites, `config.ALPACA_PAPER` unused), a stale/unwired scheduler (in-process `schedule` loop, model list missing 3 of 9 models, no crash recovery on Windows), no alerting, and zero test coverage. Addressed in one pass:

1. **Risk controls + fill verification** in `PortfolioManager`: position cap + pro-rata redistribution, structural cash buffer, turnover guard, `wait_for_order_fill` wired up (was dead code), too-few-picks abort in `StrategyTrader.get_current_picks`.
2. **Config consolidation**: `ALPACA_PAPER` is now the single source of truth; going live additionally requires `ALPACA_LIVE_TRADING_CONFIRM=YES` so a config-only flip can't silently arm live trading. New `trading/alerts.py` (SMTP email, no-ops if unset).
3. **Scheduler rework**: dropped the in-process loop for `python -m trading.scheduler --check-and-run`, meant to be triggered daily by an external scheduler; registered as Windows Task Scheduler task `FactorInvesting Rebalance` (daily 10:00, `run_scheduler_check.bat` wrapper — `schtasks /tr` couldn't take a raw `cmd /c "... && ..."` string, it mis-parsed `&&` as its own token even quoted, hence the wrapper batch file).
4. **`trade reconcile`** command (shadow-vs-actual diff).
5. **Tests**: `tests/test_portfolio_manager.py`, `tests/test_scheduler.py`.

Two bugs found live during rollout, both fixed same session:
- The turnover guard was raising even under `--dry-run`, defeating the point of previewing a trade list before committing to it. Fixed so `dry_run` only warns; the guard only actually blocks a real (non-dry-run) execution.
- `run_rebalance` was logging dry-run previews to the same JSONL file `get_last_rebalance_time()` reads for scheduler due-date state — a preview would have falsely marked a strategy as already rebalanced. Fixed to only log real executions.

A third bug surfaced on the actual first live rebalance attempt (switching the paper account from a stale SPY/AMZN/CRM holding into `six_factor`'s 30 picks): all 30 buy orders failed with Alpaca error `42210000` ("notional value must be limited to 2 decimal places") — `calculate_trades` computed notional dollar amounts as raw `weight * portfolio_value` floats (e.g. `$3789.365182...`) with no rounding. The 3 sells succeeded because full exits go through `close_position` (no notional value involved), not `submit_notional_order` — that's why the bug only showed up on the buy side. Fixed by rounding every notional value to cents in `calculate_trades`; the failure/success split was caught immediately by the new email alerting, which is exactly the scenario it was built for. Locked in by `TestNotionalRounding` in `tests/test_portfolio_manager.py`.

## Environment Variables

- `POLYGON_API_KEY` - Required for all data operations
- `ALPACA_API_KEY` / `ALPACA_SECRET_KEY` - Required for paper trading only
- `ALPACA_LIVE_TRADING_CONFIRM=YES` - Required in addition to `config.ALPACA_PAPER = False` to arm live trading (see trading layer hardening below)
- `SMTP_HOST` / `SMTP_PORT` / `SMTP_USER` / `SMTP_PASSWORD` / `ALERT_EMAIL_TO` - Optional, for `trading/alerts.py` email alerts on rebalance failures. This project reuses the same Gmail app password as `../daily_brief/.env`
